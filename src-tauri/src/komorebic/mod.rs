...
///
/// Path-discovery methods carry default `bail!` impls so that pre-existing
/// fakes (e.g. the one in `#[cfg(test)]` for `discover` / `is_running`) keep
/// compiling when new path methods are added. Real fakes that need to assert
/// path behaviour should override.
